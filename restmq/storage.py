...
from restmq.engine import RedisOperations
...
class DatabaseMixin(object):
    ...
    def setup(cls, conf):
        if "redis_settings" in conf:
            ...
            DatabaseMixin.MQ = RedisOperations(redis)
        else:
            log.msg("Redis must be enabled in restmq.conf")
            raise RuntimeError("Redis must be enabled in restmq.conf")
